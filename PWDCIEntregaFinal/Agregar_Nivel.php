...
$sql ="SELECT * from niveles where Curso = '$id'";
...
function categoryTree($parent_id = 0, $sub_mark = ''){
  ...
  if($query->num_rows > 0){
      while($row = $query->fetch_assoc()){
          echo '<li value="'.$row['idnivel'].'">'.$sub_mark.$row['nivel_nombre'].'</li>';
...
<div class="row">
    ...
    <div class="col-md-6 order-md-1">
      ...
      <form class="formulario" action="guardar_nivel.php?ID_Curso=<?php echo $filas['ID_Curso']?>" method="POST" id="form" enctype="multipart/form-data">

 
<div class="contenedor">
            <div class="input-contenedor">
                <input type="text" name="nivel_nombre" id="nivel_nombre" placeholder="Nombre Del Nivel">
           
            </div>


                <input type="submit" value="aceptar_Nivel" name="aceptar_Nivel" class="button">
...
<form class="formulario" action="guardar_subnivel.php" method="POST" id="form" enctype="multipart/form-data">

 
<div class="contenedor">
            <div class="input-contenedor">
                <input type="file" name="video" id="video" placeholder="Nombre de usuario">
           
            </div>

            <div class="input-contenedor">
            <select name="NivelPadre" id="NivelPadre" required>
                    <option value="0">Lista De Niveles</option>
                    
                    <?php
                    $query ="SELECT * FROM Niveles where Curso =$id_curso";
                    $resultado=$conn->query($query); 
                    while($row = $resultado->fetch_assoc()) { ?>
                    <option value="<?php echo $row['idnivel'];?>"><?php echo $row['nivel_nombre'];?></option>
                    <?php } ?>
                ...
                <input type="submit" value="aceptar_subnivel" name="aceptar_subnivel" class="button">
